            ...
            foreach (var type in ReverseRegistry.Keys.OrderBy(k => k.FullName))
            {
                ...
                var members = type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                    ...
                    .Select(p =>
                    {
                        ...
                        return new {Name = name, PropName = name.ToLower() + "Prop", Property = p, Type = p.PropertyType, RealName = p.Name};
                    })
                    .OrderBy(p => p.Name)
                    .ToArray();
                ...
                c.Code($"public override {type.FullName} Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {{");
                c.Code("if (reader.TokenType != JsonTokenType.StartObject)");
                c.Code("  throw new JsonException();");
                foreach (var member in members)
                {
                    c.Code($"{member.Type.FullName} {member.PropName} = default;");
                }
                ...
                foreach (var member in members)
                {
                    ...
                    c.Code($"  {member.PropName} = JsonSerializer.Deserialize<{member.Type.FullName}>(ref reader, options);");
                    c.Code("  break;");
                }
                c.Code("default:");
                c.Code("  reader.Skip();");
                ...
                foreach (var member in members)
                {
                    c.Code($"{member.RealName} = {member.PropName},");
                }
                ...
                c.Code($"public override void Write(Utf8JsonWriter writer, {type.FullName} value, JsonSerializerOptions options) {{");
                ...
                foreach (var member in members)
                {
                    ...
                    c.Code($"JsonSerializer.Serialize<{member.Type.FullName}>(writer, value.{member.RealName}, options);");
                }
